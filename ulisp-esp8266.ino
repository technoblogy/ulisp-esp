/* uLisp ESP8266 Version 2.0 beta - www.ulisp.com
   David Johnson-Davies - www.technoblogy.com - 2nd March 2018
...
#define PSTR(s) s
#define PROGMEM
#define pgm_read_word(ptr) (intptr_t)(*(ptr))
...
enum stream { SERIALSTREAM, I2CSTREAM, SPISTREAM, WIFISTREAM };
...
WITHSERIAL, WITHI2C, WITHSPI, WITHCLIENT, TAIL_FORMS, PROGN, RETURN, IF, COND, WHEN, UNLESS, AND, OR,
...
CLS, PINMODE, DIGITALREAD, DIGITALWRITE, ANALOGREAD, ANALOGWRITE, DELAY, MILLIS, WIFICONNECT, EDIT, PPRINT, 
WIFISTATUS, AVAILABLE, WIFISOFTAP, CONNECTED, WIFILOCALIP, ENDFUNCTIONS };
...
object *sp_withclient (object *args, object *env) {
  ...
  if (params == NULL) {
    ...
    else if (numberp(address)) success = client.connect(integer(address), integer(port));
...
const char ppspecial[ppspecials] PROGMEM = { IF, SETQ, TEE, LET, LETSTAR, LAMBDA, WHEN, UNLESS, WITHI2C, WITHSERIAL, WITHSPI, WITHCLIENT };
...
object *fn_wificonnect (object *args, object *env) {
  ...
  if (result == WL_CONNECT_FAILED) error(PSTR("Incorrect password"));
  if (result == WL_NO_SSID_AVAIL) error(PSTR("Network not found"));
  if (result == WL_DISCONNECTED) error(PSTR("ESP8266 not configured"));
...
const char string28[] PROGMEM = "with-client";
...
const char string152[] PROGMEM = "available";
const char string153[] PROGMEM = "wifi-server";
const char string154[] PROGMEM = "wifi-softap";
const char string155[] PROGMEM = "connected";
const char string156[] PROGMEM = "wifi-localip";
const char string157[] PROGMEM = "wifi-connect";
...
const tbl_entry_t lookup_table[] PROGMEM = {
  ...
  { string152, fn_available, 1, 1 },
  { string153, fn_wifiserver, 0, 0 },
  { string154, fn_wifisoftap, 0, 4 },
  { string155, fn_connected, 1, 1 },
  { string156, fn_wifilocalip, 0, 0 },
  { string157, fn_wificonnect, 0, 2 },
};
...
void printobject(object *form, pfun_t pfun){
  if (form == NULL) pfstring(PSTR("nil"), pfun);
    ...
    else if ((form->integer)>>8 == WIFISTREAM) pfstring(PSTR("wifi"), pfun);
...
void setup() {
  ...
  initworkspace();
  initenv();
  pfstring(PSTR("uLisp 2.0 beta "), pserial); pln(pserial);
}
